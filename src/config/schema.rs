...
pub struct UiConfig {
    ...
    #[serde(default)]
    pub color: ColorMode,
    /// Which colour depth to use before detection, or `auto`.
    #[serde(default)]
    pub color_depth: ColorDepthChoice,
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            color_depth: ColorDepthChoice::Auto,
            mouse: false,
...
/// The `ui.color-depth` value before detection.
///
/// The same four values are the configuration key, the `--color-depth` flag
/// and the serde spelling, so the accepted set cannot drift between them.
...
pub enum ColorDepthChoice {
    ...
    TrueColor,
    /// Force the 256-colour indexed palette.
    #[serde(rename = "256")]
    #[value(name = "256")]
    Indexed256,
    /// Force the 16 basic ANSI colours.
    #[serde(rename = "16")]
    #[value(name = "16")]
    Basic16,
}
...
impl ColorDepthChoice {
    /// Accepted values, for help text and error messages.
    pub const ACCEPTED: &'static [&'static str] = &["auto", "truecolor", "256", "16"];
}
