...
| A cloud tool's error text carries a credential or terminal escapes into a message | Its standard error is redacted through the one existing implementation before display, including inline authorization header values and bare bearer tokens | Redaction is pattern-based; a folded or unusual credential shape may still pass |
