...
| 002 Connection experience and secrets | Mostly done | Profiles, password files, service files, TLS states, connection diagnostics and a password prompt in the client are in (`specs/009-connection-profiles/spec.md`). Remaining: an OS credential store, a first-run journey, and a password prompt in plain mode |
